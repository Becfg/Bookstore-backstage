<template>
  <el-aside :class="isCollapse ? 'el-aside--collapse' : ''">
    <el-menu :default-active="activePath" class="el-menu-aside" :collapse="isCollapse" :router="true"
      :collapse-transition="false" @open="handleOpen" @close="handleClose">
      <div class="sidebar-logo-container">
        <el-icon>
          <IconLogo />
        </el-icon>
        <span v-if="!isCollapse"> 书店后台管理系统 </span>
      </div>
      <MenuItem v-for="item in menus" v-show="item.meta?.['showMenu']" :data="item" />
    </el-menu>
  </el-aside>
</template>
...
import { getbookCategoryList } from '@/apis/book/bookCategory'
import { getBookList } from '@/apis/book/bookList'
